...
def two_sum(nums, target)
  ...
  last = 0
  pos_dict = {}
  nums.each_index { |i| pos_dict[nums[i]] = [] if pos_dict[nums[i]].nil? ;pos_dict[nums[i]] << i}
  nums_sorted = nums.sort
  nums_sorted.each_index do |i|
    first = i
    last = find_match(nums_sorted,i+1,nums_sorted.size-i-1,nums_sorted[i],target)
    break if last > 0
  end
  return [0,0] if last == 0
  first_pos = pos_dict[nums_sorted[first]][0]
  pos_dict[nums_sorted[first]].delete_at 0
  last_pos = pos_dict[nums_sorted[last]][0]
  pos_dict[nums_sorted[last]].delete_at 0
  [first_pos,last_pos]
end

def find_match(nums,begin_pos,length,first,target)
  # p "#{begin_pos},#{length} #{first} #{nums[begin_pos+length/2]}"
  return 0 if begin_pos >= nums.size
  return begin_pos if nums[begin_pos] + first == target
  return 0 if nums[begin_pos] + first != target && length == 0
  if length == 1
    return 0 if length + begin_pos >= nums.size
    return 0 if first + nums[begin_pos+length] != target
    return begin_pos+length if first + nums[begin_pos+length] == target
  end
  return begin_pos+length/2 if nums[begin_pos+length/2] + first == target
  return find_match(nums,begin_pos,length/2,first,target) if nums[begin_pos+length/2] + first > target
  return find_match(nums,begin_pos+length/2,length/2,first,target) if nums[begin_pos+length/2] + first < target
  0
end
